(function (H) {
	...
	seriesTypes.trendline = H.extendClass(seriesTypes.line, {
		...
		**/
		SMA: function (xData, yData, periods) {
			...
			// Only calculate the data in current range
			var min = this.linkedParent.xAxis.min,
				max = this.linkedParent.xAxis.max,
			...
			return SMA(xData, yData, periods, minIndex, maxIndex);
		},
	...
	**/
	var cachedSMA = [];
	function SMA (xData, yData, periods, minIndex, maxIndex) {
		var periodArr = [],
			smLine = [],
			length = yData.length,
			start = 0,
		...
		for (var i = start; i < length; i++) {
			...
			if (periods == periodArr.length) {
				if (cachedSMA[xData[i]]) {
					smLine.push([ xData[i], cachedSMA[xData[i]]]);
					continue;
				}
				...
				smLine.push([ xData[i] , y]);
				periodArr.splice(0,1);
				cachedSMA.push(cacheData);
